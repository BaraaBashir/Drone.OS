...
void RunChildProcess(int index, int n, int write_fd) {
	...
	double accumulated_sim_time = 0.0;
	...
	for (int step = 0; step < childPathCount; step++) {
		...
		msg.virtual_time = accumulated_sim_time;
		...
		if (!msg.is_finished) {
			...
			accumulated_sim_time += (W * 0.3) + 1.0;
		...
		if (isPlaying) {
			...
			while (read(pipe_fds[0], &msg, sizeof(IPCMessage)) > 0) {
				...
				if (msg.is_finished) {
					...
					gettimeofday(&current_real_time, NULL);
					
					double real_time = (current_real_time.tv_sec - start_real_time.tv_sec) + 
									   (current_real_time.tv_usec - start_real_time.tv_usec) / 1000000.0;
					
					double simulated_time = msg.virtual_time;
					double diff = real_time - simulated_time;

					printf("Drone [%d] arrived! Simulated time: %.2f, Real time: %.2f (Diff: %.2f)\n", 
						   msg.pid, simulated_time, real_time, diff);
				} else {
